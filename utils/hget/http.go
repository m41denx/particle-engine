...
var (
	tr = &http.Transport{
		TLSClientConfig: &tls.Config{InsecureSkipVerify: false}, // Unsure if this will work
	}
...
func NewHttpDownloader(url string, par int, skipTls bool) *HttpDownloader {
	...
	ips, err := net.LookupIP(strings.Split(parsed.Host, ":")[0])
	FatalCheck(err)
	...
	if clen == "1" {
		fmt.Println(color.GreenString(" • ? MB"))
	} else if sizeInMb < 1024 {
		fmt.Println(color.GreenString(" • %.1f MB", sizeInMb))
	} else {
		fmt.Println(color.GreenString(" • %.1f GB", sizeInMb/1024))
	}
